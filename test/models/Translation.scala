...
class TranslationSpec extends Specification {
  ...
  "create some translation" in {
    running(FakeApplication(additionalConfiguration = inMemoryDatabase())) {
      ...
      Translation.create(idIRI.get,idLang.get,"Persona",1)
      Translation.create(idIRI.get,idLang.get,"Gente",3)
	  ...
	  val t = Translation.findById(idt.get)
	  t must beSome
	  t.get.transLabel must beEqualTo("Gente")
    }
   }
  
  "delete all translation" in {
    running(FakeApplication(additionalConfiguration = inMemoryDatabase())) {
      val langCode = "es"
      val langName = "Spanish"
      Language.create(langCode,langName)
      ...
      Translation.all().size must be_==(2)
      Translation.deleteALL()
      Translation.all().size must be_==(0)
